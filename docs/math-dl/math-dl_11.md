...
这个实验，我只呈现结果，训练了一个小型卷积神经网络，使用16,384个随机样本作为训练集，批量大小为128，训练12个周期。结果展示了每个优化器（SGD、RMSprop、Adagrad 和 Adam）五次运行的均值和标准误差。关注点是测试集的准确率和训练的时钟时间。我在同一台机器上训练了所有模型，所以我们应该关注相对时间。未使用 GPU。
