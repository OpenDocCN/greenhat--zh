...
本附录展示了*Escape*游戏的最终列表。你可以将其作为参考，查看各个函数和部分应放置的位置，或者如果你想在一个地方查看完整的列表，也可以通读一遍。这个列表不包括你在构建游戏时写的临时部分，比如`EXPLORER`部分。它仅包含最终游戏中的代码。

记住，你也可以下载*escape.py*列表并在 IDLE 中阅读，使用 CTRL-F 进行搜索。

我在这个列表中将`PLAYER_NAME`更改为“Captain”。在你构建或自定义游戏时，你可以使用你自己的名字（见第 4-1 列表中的➊，位于第 63 页）。
